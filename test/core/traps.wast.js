
// traps.wast:5
let $1 = instance("");

// traps.wast:16
assert_trap(() => call($1, "no_dce.i32.div_s", [1, 0]));

// traps.wast:17
assert_trap(() => call($1, "no_dce.i32.div_u", [1, 0]));

// traps.wast:18
assert_trap(() => call($1, "no_dce.i64.div_s", [1n, 0n]));

// traps.wast:19
assert_trap(() => call($1, "no_dce.i64.div_u", [1n, 0n]));

// traps.wast:20
assert_trap(() => call($1, "no_dce.i32.div_s", [-2_147_483_648, -1]));

// traps.wast:21
assert_trap(() => call($1, "no_dce.i64.div_s", [-9_223_372_036_854_775_808n, -1n]));

// traps.wast:23
let $2 = instance("");

// traps.wast:34
assert_trap(() => call($2, "no_dce.i32.rem_s", [1, 0]));

// traps.wast:35
assert_trap(() => call($2, "no_dce.i32.rem_u", [1, 0]));

// traps.wast:36
assert_trap(() => call($2, "no_dce.i64.rem_s", [1n, 0n]));

// traps.wast:37
assert_trap(() => call($2, "no_dce.i64.rem_u", [1n, 0n]));

// traps.wast:39
let $3 = instance("");

// traps.wast:50
assert_trap(() => call($3, "no_dce.i32.trunc_f32_s", [float32(2_143_289_344)]));

// traps.wast:51
assert_trap(() => call($3, "no_dce.i32.trunc_f32_u", [float32(2_143_289_344)]));

// traps.wast:52
assert_trap(() => call($3, "no_dce.i32.trunc_f64_s", [float64(9_221_120_237_041_090_560n)]));

// traps.wast:53
assert_trap(() => call($3, "no_dce.i32.trunc_f64_u", [float64(9_221_120_237_041_090_560n)]));

// traps.wast:54
assert_trap(() => call($3, "no_dce.i64.trunc_f32_s", [float32(2_143_289_344)]));

// traps.wast:55
assert_trap(() => call($3, "no_dce.i64.trunc_f32_u", [float32(2_143_289_344)]));

// traps.wast:56
assert_trap(() => call($3, "no_dce.i64.trunc_f64_s", [float64(9_221_120_237_041_090_560n)]));

// traps.wast:57
assert_trap(() => call($3, "no_dce.i64.trunc_f64_u", [float64(9_221_120_237_041_090_560n)]));

// traps.wast:59
let $4 = instance("");

// traps.wast:78
assert_trap(() => call($4, "no_dce.i32.load", [65_536]));

// traps.wast:79
assert_trap(() => call($4, "no_dce.i32.load16_s", [65_536]));

// traps.wast:80
assert_trap(() => call($4, "no_dce.i32.load16_u", [65_536]));

// traps.wast:81
assert_trap(() => call($4, "no_dce.i32.load8_s", [65_536]));

// traps.wast:82
assert_trap(() => call($4, "no_dce.i32.load8_u", [65_536]));

// traps.wast:83
assert_trap(() => call($4, "no_dce.i64.load", [65_536]));

// traps.wast:84
assert_trap(() => call($4, "no_dce.i64.load32_s", [65_536]));

// traps.wast:85
assert_trap(() => call($4, "no_dce.i64.load32_u", [65_536]));

// traps.wast:86
assert_trap(() => call($4, "no_dce.i64.load16_s", [65_536]));

// traps.wast:87
assert_trap(() => call($4, "no_dce.i64.load16_u", [65_536]));

// traps.wast:88
assert_trap(() => call($4, "no_dce.i64.load8_s", [65_536]));

// traps.wast:89
assert_trap(() => call($4, "no_dce.i64.load8_u", [65_536]));

// traps.wast:90
assert_trap(() => call($4, "no_dce.f32.load", [65_536]));

// traps.wast:91
assert_trap(() => call($4, "no_dce.f64.load", [65_536]));
